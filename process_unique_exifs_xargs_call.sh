...
source /Users/shinka/Userscripts/process_unique_exifs_xargs_main.sh
...
function processPhoto() {
  ...
  if [ "$EXPORT_MODE" = true ]; then
	...
	if [ -n "$FILE_ORI_TIME" ] && [ ${#FILE_ORI_TIME} -gt 7 ]; then
          NEW_FILE_NAME="LUDWIG_ORI${FILE_ORI_TIME}_MOD${FILE_MOD_TIME}.${IMAGE_EXTENSION}"
	  ...
	  DATE=$(echo "$FILE_ORI_TIME" | grep -oE "^[0-9]+")
          ...
          NEW_FILE_NAME="LUDWIG_ORI-ISEMPTY_MOD${FILE_MOD_TIME}.${IMAGE_EXTENSION}"
